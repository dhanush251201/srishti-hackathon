...
const AdminWrapper = () => {
  ...
  return (
    <main className="h-screen w-screen overflow-auto flex font-sans">
      <nav className="bg-gradient-to-bl from-teal-600 to-black shadow-lg w-1/4 overflow-hidden py-8 h-screen">
        ...
        <div className="flex flex-col w-full h-[calc(100vh-10rem)]">
          {menuItems.map((item, idx) => {
            return item.text !== "default" ? (
              <Link to={item.link} key={idx}>
                <div className="text-white text-xl mb-4 flex items-center space-x-4 px-8 hover:bg-cloud hover:text-teal-600 py-2">
                  {item.icon}
                  <p>{item.text}</p>
                </div>
              </Link>
          ...
          <button
            ...
            className="text-white text-xl mb-4 flex items-center space-x-4 px-8 hover:bg-cloud hover:text-teal-600 py-2"
          >
            <MdLogout />
            <p>Logout</p>
          </button> 
